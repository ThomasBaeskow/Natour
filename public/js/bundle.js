...
var hideAlert = function hideAlert() {
  var el = document.querySelector(".alert"); // we select the div with the "alert" class
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return (0, _axios.default)({
              method: "PATCH",
              url: url,
              // the route in backend which we want to update ( where our controller UpdateMe is mounted). We update the name and email in our DB of the current User
              data: data
            });
          case 4:
            result = _context.sent;
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(tourId) {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // the tourId comes from tour.pug where we stored it in (data-tour-id=`${tour.id}`)
          ...
          case 4:
            ...
            return stripe.redirectToCheckout({
              // we redirect the user to stripe by passing in as options an object, which holds property value pair sessionId of stripe. Its stored inside session.data.session.id
          ...
          case 10:
            ...
            (0, _alerts.showAlert)("error", _context.t0); // shw the user as an alert, when there is an error
          case 14:
          case "end":
            return _context.stop();
        }
...
// DOM ELEMENTS
var mapBox = document.getElementById('map'); // Thats the map the created with "mapbox"
var loginForm = document.querySelector('.form--login'); // thats the login form with 2 inputs (email, password)
var logOutBtn = document.querySelector('.nav__el--logout');
var userDataForm = document.querySelector(".form-user-data"); // thats the form which holds the inputs (data to update from the frontend)
var userPasswordForm = document.querySelector(".form-user-password");
var bookBtn = document.getElementById('book-tour'); // we select the button in tour.pug with id= book-tour
...
if (loginForm) {
  loginForm.addEventListener('submit', function (e) {
    ...
    e.preventDefault();
    // VALUES
    var email = document.getElementById('email').value; // we get the data from the client side. Input field has the id="email"
    var password = document.getElementById('password').value; // we get the data from the client side. Inputfield has the id="password"
    (0, _login.login)(email, password); // we are passing in the email and the password which the user gives us from the client side.
  });
}

if (logOutBtn) logOutBtn.addEventListener("click", _login.logout); // we add eventListener on the element which has class ".nav__el--logout" at the "click" event. We logout!
...
if (bookBtn) bookBtn.addEventListener("click", function (e) {
  ...
  var tourId = e.target.dataset.tourId; // JS is converting the id "tour-id" to tourId. Thats why we need to target with tourId. We sore that id in variable.
  (0, _stripe.bookTour)(tourId); // we pass in as parameter the tourId. Booktour will make an API request to the backends route: await axios(`http://127.0.0.1:3000/api/v1/booking/checkout-session/${tourId}`) ==> which returns a checkout session (payment).
});
